...
export class ModuleTicketingPublic { 	//  FlowSystem ticketing
  constructor(
  ...
  statusTicketing = [
    ...
    { status: "wait-sched-surv", changeStatus: "Waiting For Schedule Survey", stepByFlow: "sched-for-surv" }, // Step No.2
    ...
    { status: "surv-work-done", changeStatus: "Survey Done", stepByFlow: 'wait-sched-wo' }, // Step No.6
    ...
    // Handled By "TRO"
    { status: "wait-sched-wo", changeStatus: "Waiting For Schedule WO", stepByFlow: "sched-for-fix" },   // Step No.9
  ...
  sendHandledBy(status: string, value: any, controlsValue: any) {
    const result = status === "wait-sched-surv" || status === "sched-for-fix" //Condition Handled By
      || status === "wait-sched-wo" ? "mob" : status === "tick-cust-approve" || status === "open" || status === "surv-work-done" ? "spv" : status === "wait-confirm-wo" || status === "surv-work-done" ? "tro" : status === "wait-confirm-wo" ? value : controlsValue
    return result
  }
