...
export class BillingEffect {
	...
	loadBillingPage$ = this.actions$.pipe(
		...
		map((response) => {
			let res: { errorMessage: string; allTotalCount:any; totalCount: any; allBillingAmount:any; items: any };
			const result: QueryResultsModelUpd = {
				items: response[0].data,
				totalCount: response[0].totalCount,
				allTotalCount : response[0].allTotalCount,
				allBillingAmount: response[0].allBillingAmount,
				errorMessage: "",
				data: [],
			};

			const lastQuery: QueryBillingModelUpd = response[1];
	...
	loadBillingPageLog$ = this.actions$.pipe(
		...
		map((response) => {
			let res: { errorMessage: string; allTotalCount:any; allBillingAmount:any; totalCount: any; items: any };
			const result: QueryResultsModelUpd = {
				items: response[0].data,
				totalCount: response[0].totalCount,
				allTotalCount: response[0].allTotalCount,
