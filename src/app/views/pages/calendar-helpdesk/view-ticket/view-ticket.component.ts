...
import { Popup, PopupReschdule, PopupReschduleFixing } from '../../ticket/popup/popup.component';
...
export class ViewTicketComponent implements OnInit, OnDestroy {
	...
	@ViewChild("modalviewticket", { static: true }) modalviewticket: ElementRef;
	constructor(
	...
	initTicket() {
		...
		this.loadEnginer();
		this.hiddenAfterRes();
		this.getUnitId(this.ticket.unit._id)
	}
	...
	async loadDataId(id) {
		const URL_DATA = `${environment.baseAPI}/api/ticket`
	...
	openLarge() {
		this.modalService.open(this.modalviewticket, {
			centered: true,
			size: 'sm',
			backdrop: "static",
		});
