...
@Component({
  selector: 'kt-add-poReceipt',
  templateUrl: './add-poReceipt.component.html',
  styleUrls: ['./add-poReceipt.component.scss']
})
export class AddpoReceiptComponent implements OnInit, OnDestroy {
	...
	displayedColumns = [
		{def: "product_brand", hide: false},
		{def: "qty", hide: false},
		{def: "in_history", hide: true},
		{def: "in", hide: false},
		{def: "return", hide: false},
		{def: "uom", hide: false},
		{def: "price", hide: false},
		{def: "in_price", hide: false},
		// "product_brand", "qty","in", "return","uom", "price", "in_price"
	];
	...
	createForm() {
		this.poReceiptForm = this.poReceiptFB.group({
			...
			po_no: [{value: undefined, disabled: false}, Validators.required],
			po: [{value: undefined, disabled: false}, Validators.required],
			type_receipt: [{value: undefined, disabled: false}, Validators.required],
			status: [{value: "", disabled: true}, Validators.required],
			DO_number: [{value: "", disabled: false}, Validators.required],
			courier_name: [{value: "", disabled: false}, Validators.required],
			description: [{value: "", disabled: false}, Validators.required],
			vendorAddress: [{value: "", disabled: false}, Validators.required],
			vendor: [{value: undefined, disabled: true}, Validators.required],
			type_po: [{value: "", disabled: false}, Validators.required],
			
			products: this.poReceiptFB.array([this.poReceiptFB.group({
	...
	getDisplayedColumns() {
		return this.displayedColumns.filter(item => !item.hide).map(item => item.def)
	}
	...
	vendorchange(value){
		...
		this.serviceVendor.findVendorById(value._id).subscribe(
			res => {
				...
				this.poReceiptForm.controls.vendorDesc.setValue(data.description)
	...
	addpoReceipt( _poReceipt, withBack: boolean = false) {
		const addSubscription = this.service.createPoReceipt(_poReceipt).subscribe(
			...
			err => {
				console.error(err, 'eeror');
				const message = 'Error while adding PO receipt | ' + err.error.message;
	...
	poonChange(value){
		...
		this.servicePO.findPurchaseOrderById(value._id).subscribe(res => {
			...
			//get POR history in
			this.service.calculateHistoryIn(value._id).subscribe(result => {

				var dataHistory = result.data

				console.log(dataHistory, 'data his')
				...
				data.product.map((el, idx) =>
					{
						...
						if(dataHistory.length > 0){
							if(el.product._id === dataHistory[idx].product){
								history_in = dataHistory[idx].in_received
								this.displayedColumns[2].hide = false
							}
						}
						productsArray.push(this.poReceiptFB.group({
							...
							in_qty: new FormControl(),
							return_qty: new FormControl(),
							history_in: new FormControl(history_in),
							in_price: new FormControl()
						})
						)
					} 
	...
	isFullQuantity() {
		let productsArray = this.poReceiptForm.controls['products'] as FormArray
		var isFull = true
		productsArray.value.map(item => {
			if(item.in_qty + item.history_in < item.qty) isFull = false
		})

		if(isFull) this.poReceiptForm.controls['type_receipt'].setValue('1')
	}
	...
	onChangeProduct(e, id, prop) {
		...
		let productsArray = this.poReceiptForm.controls['products'] as FormArray
		if(prop === 'in_qty'){
			productsArray.at(id).patchValue({
				in_price: e.target.value * productsArray.at(id).get('price').value
			})
		}
		...
		if(control.type_po.value === "in" && control.type_receipt.value === "1"){
			productsArray.controls.forEach((el, id) => {
				if(el instanceof FormGroup){
					el.controls.in_qty.setValue(el.value.qty - parseInt(el.value.history_in))
					el.controls.return_qty.setValue(0)

					el.controls.in_price.setValue((el.value.qty - parseInt(el.value.history_in)) * el.value.price)
				}
			});
		}
			...
			productsArray.controls.forEach((el, id) => {
				if(el instanceof FormGroup){
					el.controls.return_qty.setValue(el.value.qty - parseInt(el.value.history_in))
					el.controls.in_qty.setValue(0)

					el.controls.in_price.setValue(0 * el.value.price)

					//if return - full, disable in qty form
	...
	typePOChange(value) {
		...
		//hide return column
		if(control.type_po.value === "in"){
			this.displayedColumns[4].hide = true
		}
		else{
			this.displayedColumns[4].hide = false
		}
