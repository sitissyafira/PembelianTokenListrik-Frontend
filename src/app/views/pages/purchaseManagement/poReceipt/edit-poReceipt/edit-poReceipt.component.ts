...
@Component({
  selector: 'kt-edit-poReceipt',
  templateUrl: './edit-poReceipt.component.html',
  styleUrls: ['./edit-poReceipt.component.scss']
})
export class EditpoReceiptComponent implements OnInit, OnDestroy {
	...
	createForm() {
		this.poReceiptForm = this.poReceiptFB.group({
			...
			po: [{value: this.poReceipt.po._id, disabled: true}, Validators.required],
			type_receipt: [{value: this.poReceipt.type_receipt, disabled: false}, Validators.required],
			status: [{value: this.poReceipt.status, disabled: true}, Validators.required],
			DO_number: [{value: this.poReceipt.DO_number, disabled: false}, Validators.required],
			courier_name: [{value: this.poReceipt.courier_name, disabled: false}, Validators.required],
			description: [{value: this.poReceipt.description, disabled: false}, Validators.required],
			...
			type_po: [{value: this.poReceipt.type_po, disabled: false}, Validators.required],
	...
	onSubmit(withBack: boolean = false) {
		...
		this.loading = true;
		let editedpoReceipt = this.preparepoReceipt();
	...
	preparepoReceipt() {
		...
		form.append("po", controls.po.value)
	...
	updatepoReceipt(_poReceipt, withBack: boolean = false) {
		const addSubscription = this.service.updatePoReceipt(this.poReceipt._id, _poReceipt).subscribe(
			res => {
			...
			err => {
				console.error(err);
				const message = 'Error while adding purchase order | ' + err.error.message;
		...
		if(control.type_po.value === "in" && control.type_receipt.value === "1"){
			productsArray.controls.forEach((el, id) => {
				if(el instanceof FormGroup){
					...
					el.controls.in_price.setValue(el.value.qty * el.value.price)
				}
			});
		}
