...
@Component({
  selector: 'kt-edit-paymentPo',
  templateUrl: './edit-paymentPo.component.html',
  styleUrls: ['./edit-paymentPo.component.scss']
})
export class EditpaymentPoComponent implements OnInit, OnDestroy {
	...
	displayedColumnsAp = ['no', 'gl', 'amount', 'desc', 'isdebit', 'action'];
	...
	createForm() {
		...
		this.paymentPoForm = this.paymentPoFB.group({
			...
			purchase_invoice_no: [{value: this.paymentPo.purchase_invoice_no, disabled: false}, Validators.required],
			...
			ppn: [{value: this.selectedPO.payment.ppn, disabled: false}, Validators.required],
			pph: [{value: this.selectedPO.payment.pph, disabled: false}, Validators.required],
			date: [{value: this.paymentPo.date, disabled: false}, Validators.required],
			planned_payment_date: [{value: this.paymentPo.planned_payment_date, disabled: false}, Validators.required],
			...
			notes: [{value: this.paymentPo.note, disabled: false}, Validators.required],
			...
			ppnPOR: [{value: this.toRupiahString(this.paymentPo.payment.ppn), disabled: false}, Validators.required],
			pphPOR: [{value: this.toRupiahString(this.paymentPo.payment.pph), disabled: false}, Validators.required],
			discountValuePOR: [{value: this.toRupiahString(this.paymentPo.payment.discountValue), disabled: false}, Validators.required],
			shippingCostPOR: [{value: this.toRupiahString(this.paymentPo.payment.shippingCost), disabled: false}, Validators.required],
	...
	initAp() {
		...
		for (let i = 1; i <= 30; i++) {
			...
			if (this.ap.AP.multiGLAccount[`memo${i}`] !== undefined || this.ap.AP.multiGLAccount[`memo${i}`] !== null) memo = this.ap.AP.multiGLAccount[`memo${i}`];
		...
		this.apForm = this.apFB.group({
			depositTo: [{value: this.ap.AP.depositTo, disabled: false}],
			voucherno: [{value: this.ap.AP.voucherno, disabled: true}, Validators.required],
			rate: [{ value: this.ap.AP.rate, disabled: false }], //number
			date: [{value: this.ap.AP.date, disabled: false}, Validators.required], // number
			...
			status: [{value: this.ap.AP.status, disabled: false}],
			paidFrom: [{ value: this.ap.AP.paidFrom._id, disabled: false }],
			multiGLAccount: this.apFB.group(objFormMultiGl),
			payTo: [{value: this.ap.AP.payTo, disabled: false}],
			payee: [{value: this.ap.AP.payee, disabled: false}],
			noAccount: [{value: this.ap.AP.noAccount, disabled: false}],
			bank: [{value: this.ap.AP.bank == null ? "" : this.ap.AP.bank._id, disabled: false}],
		});
	...
	preparepaymentPo() {
		...
		for (let i = 2; i <= 30; i++) {
			...
			objRes[`amount${i}`] = apControls.multiGLAccount.get(`amount${i}`).value == "" || apControls.multiGLAccount.get(`amount${i}`).value == undefined || apControls.multiGLAccount.get(`amount${i}`).value == null ? undefined : parseFloat(rupiahNum);
			objRes[`isDebit${i}`] = apControls.multiGLAccount.get(`amount${i}`).value == "" ? undefined : apControls.multiGLAccount.get(`isDebit${i}`).value
		}
		apData = {
			...apData,
			_id: this.paymentPo.ap._id,
			totalField: this.totalField,
			glaccount: apControls.multiGLAccount.get('glAccountId1').value,
			amount: apControls.multiGLAccount.get('amount1').value == "" ? undefined : parseFloat(apControls.multiGLAccount.get('amount1').value.replace(/[\.]+/g, "").replace(/[\,]+/g, ".")),
