...
export class ListTicketScheduledComponent implements OnInit, OnDestroy {
	file;
	...
	navMatTab: any[] = [
		...
		{ name: "Scheduled", value: "scheduled", tab: true },
	...
	matTabSelection(status) {
		if (status === "all") this.router.navigateByUrl(`/publicSpvTicket`, { relativeTo: this.activatedRoute });
		else if (status === "open") this.router.navigateByUrl(`/publicSpvTicket/open`, { relativeTo: this.activatedRoute });
		else if (status === "done") this.router.navigateByUrl(`/publicSpvTicket/done`, { relativeTo: this.activatedRoute });
		else if (status === "waitConfirm") this.router.navigateByUrl(`/publicSpvTicket/wfc`, { relativeTo: this.activatedRoute });
		else if (status === "waitSurvey") this.router.navigateByUrl(`/publicSpvTicket/wfsurvey`, { relativeTo: this.activatedRoute });
		else if (status === "scheduled") this.router.navigateByUrl(`/publicSpvTicket/sch`, { relativeTo: this.activatedRoute });
		else if (status === "reschedule") this.router.navigateByUrl(`/publicSpvTicket/rsh`, { relativeTo: this.activatedRoute });
		else if (status === "waitSch") this.router.navigateByUrl(`/publicSpvTicket/wfs`, { relativeTo: this.activatedRoute });
		else if (status === "schSurvey") this.router.navigateByUrl(`/publicSpvTicket/schSurvey`, { relativeTo: this.activatedRoute });
		else if (status === "reject") this.router.navigateByUrl(`/publicSpvTicket/reject`, { relativeTo: this.activatedRoute });
		else if (status === "svyDone") this.router.navigateByUrl(`/publicSpvTicket/svyDone`, { relativeTo: this.activatedRoute });
		else if (status === "scheduled") this.router.navigateByUrl(`/publicSpvTicket/sch`, { relativeTo: this.activatedRoute });
	}
	refresh() {
		this.loadTicketList()
	...
	loadTicketList(forSearch?: boolean) {
		...
		const queryParams = new QueryPublicTicketModelUpd(
			...
			this.paymentValue,
			["sched-for-surv", "sched-for-fix", "resched-for-surv", "resched-for-fix"],
			"spv",
			this.priorityValue
		);
	...
	editTicket(id) {
		const url = `/publicSpvTicket/edit/${id}`;
		this.router.navigateByUrl(url, { relativeTo: this.activatedRoute });
	}

	viewTicket(id) {
		const url = `/publicSpvTicket/view/${id}`;
		this.router.navigateByUrl(url, { relativeTo: this.activatedRoute });
	}
