...
import { Popup, PopupReschdule, PopupReschduleFixing, PopupRefixing } from '../popup/popup.component';
...
export class EditTicketComponent implements OnInit, OnDestroy {
	...
	createForm() {
		...
		this.ticketForm = this.ticketFB.group({
			...
			priority: [{ value: this.ticket.priority, disabled: this.ticket.priority? true : false }],
			...
			// New Flow
			paidTicket: [{ value: false, disabled: false }],
	...
	processRefixing(id) {
		const controls = this.ticketForm.controls;
		const dialogRef = this.dialog.open(PopupRefixing, {
			data: {
				input: ""
			},
			maxWidth: "400px",
		...
		dialogRef.afterClosed().subscribe((result) => {
			const _ticket = new PublicTicketModel();
			_ticket._id = this.ticket._id;
			_ticket.status = "wait-sched-wo";
			_ticket.reason = result.reason;
			_ticket.ticketId = controls.ticketId.value;
			_ticket.createdDate = controls.createdDate.value;
			_ticket.contract = controls.contract.value;
			_ticket.contract2 = controls.contract2.value;
			...
			_ticket.attachment = controls.attachment.value;
			_ticket.updatedBy = this.idEdited;
			_ticket.handledBy = "tro"


			this.service.updatePublicTicket(_ticket).subscribe(
				res => {
					const message = `Ticket successfully has been refixing.`;
					this.layoutUtilsService.showActionNotification(message, MessageType.Update, 5000, true, true);
	...
	toggleIsPaid(event) {
		this.ticketForm.controls.paidTicket.setValue(event.target.checked);
	}
	...
	async getReport() {
		const URL_DO = `${environment.baseAPI}/api/doPublic/get/do`
		await this.http.get(`${URL_DO}/${this.ticket._id}`).subscribe((res: any) => {
