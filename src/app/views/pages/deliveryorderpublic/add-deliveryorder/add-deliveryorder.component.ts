...
export class AddDoComponent implements OnInit, OnDestroy {
	...
	hiddenAfterRes() {
		if (this.publicDeliveryorder.rescheduleDateSurvey == null) {
			this.buttonRes = true;
		} else {
	...
	createForm() {
		...
		this.deliveryorderForm = this.deliveryorderFB.group({
			...
			attachment: [this.publicDeliveryorder.attachmentFixing],
			...
			// New Flow
			handledBy: [{ value: this.publicDeliveryorder.handledBy ? this.publicDeliveryorder.handledBy : "", disabled: true }],
	...
	submitForm() {
		...
		formData.append("updatedBy", this.publicDeliveryorder.updatedBy);
		formData.append("status", this.deliveryorderForm.get("status").value);
		formData.append("isRead", false);
		formData.append("handledBy", this.publicDeliveryorder.status === "vis-for-fix" ? "spv" : this.deliveryorderForm.get("handledBy").value);

		this.http
			.patch(`${this.API_URL_DO}/${this.publicDeliveryorder._id}`, formData)
			.subscribe(
				(response) => {
