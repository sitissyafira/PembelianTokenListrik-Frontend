...
import { TransactionsDeleted, TransactionsPageRequested } from '../../../../core/transactions/transactions.action';
import { TransactionsDataSource } from '../../../../core/transactions/transactions.datasource';
import { SubheaderService } from '../../../../core/_base/layout';
import { TransactionsService } from '../../../../core/transactions/transactions.service';
import { HttpClient } from '@angular/common/http';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { QueryTransactionsModel } from '../../../../core/transactions/querytransactions.model';
...
import { TransactionsGenerateDialogComponent } from './generate-transactions/generate-transactions.dialog.component';
import { MatDialog } from '@angular/material';

@Component({
	selector: 'kt-list-transactions',
...
export class ListTransactionsComponent implements OnInit, OnDestroy {
	...
	transactionsResult: TransactionsModel[] = [];
	...
	ngOnInit() {
		...
		**/
		...
		this.subheaderService.setTitle('Transactions');

		// Init DataSource
		this.dataSource = new TransactionsDataSource(this.store);
	...
	loadTransactionsList() {
		...
		const queryParams = new QueryTransactionsModel(
		...
		this.store.dispatch(new TransactionsPageRequested({ page: { ...queryParams, fromDate: this.filterStartDateControl.value, toDate: this.filterEndDateControl.value, sortOrder: this.sortOrder, sortField: this.sortField } }));
		this.selection.clear();
	}
	...
	deleteTransactions(_item: TransactionsModel) {
		// tslint:disable-next-line:variable-name
		const _title = 'Transactions Delete';
		// tslint:disable-next-line:variable-name
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.store.dispatch(new TransactionsDeleted({ id: _item._id }));
	...
	isAllSelected(): boolean {
		...
		const numRows = this.transactionsResult.length;
		return numSelected === numRows;
	}

	masterToggle() {
		if (this.selection.selected.length === this.transactionsResult.length) {
			this.selection.clear();
		} else {
			this.transactionsResult.forEach(row => this.selection.select(row));
		}
	}
	...
	detailItem(id: string) {
		const url = `/transactions/detail/${id}`;
	...
	onSubmit() {
		...
		this.http.post<any>(`${environment.baseAPI}/api/excel/transactions/import`, formData).subscribe(
			res => {
