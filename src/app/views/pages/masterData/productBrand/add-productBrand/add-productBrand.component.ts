...
@Component({
  selector: 'kt-add-productBrand',
  templateUrl: './add-productBrand.component.html',
  styleUrls: ['./add-productBrand.component.scss']
})
export class AddProductBrandComponent implements OnInit, OnDestroy {
  	...
  	ngOnInit() {
		...
		const routeSubscription =  this.activatedRoute.params.subscribe(params => {
			...
			if (id) {
				...
				this.productBrand = new ProductBrandModel();
				this.productBrand.clear();
				this.initProductBrand();
			}
		});
	...
	onSubmit(withBack: boolean = false) {
		...
		if (editedProductBrand._id) {
			this.updateProductBrand(editedProductBrand, withBack);
			return;
		}
		this.addProductBrand(editedProductBrand, withBack);
	}
	...
	addProductBrand( _productBrand: ProductBrandModel, withBack: boolean = false) {
		const addSubscription = this.service.createProductBrand(_productBrand).subscribe(
			res => {
				const message = `New product brand successfully has been added.`;
	...
	getComponentTitle() {
		let result = 'Create Product Brand';
		if (!this.productBrand || !this.productBrand._id) {
			return result;
		}

		result = `Edit Product Brand`;
		return result;
	}
