...
@Component({
  selector: 'kt-edit-stockProduct',
  templateUrl: './edit-stockProduct.component.html',
  styleUrls: ['./edit-stockProduct.component.scss']
})
export class EditStockProductComponent implements OnInit, OnDestroy {
	...
	private subscriptions: Subscription[] = [];
  	...
  	ngOnInit() {
		...
		const routeSubscription =  this.activatedRoute.params.subscribe(params => {
			const id = params.id;
			if (id) {
				this.store.pipe(select(selectStockProductById(id))).subscribe(res => {
					if (res) {
						this.stockProduct = res;
						console.log(res);
						this.oldStockProduct = Object.assign({}, this.stockProduct);
	...
	initStockProduct() {
        ...
        this.loadProductBrand(this.stockProduct.product_category._id)
		console.log(this.stockProduct.product_category._id)
        this.loadUOM();
	...
	createForm() {
			this.stockProductForm = this.stockProductFB.group({
				product_category: [this.stockProduct.product_category._id, Validators.required],
				product_brand: [this.stockProduct.product_brand._id, Validators.required],
				product_code: [this.stockProduct.product_code, Validators.required],
				product_name: [this.stockProduct.product_name, Validators.required],
				stock_qty: [{value:this.stockProduct.available_qty, disabled:true}],
				uom : [this.stockProduct.uom._id, Validators.required],
				buy_price : [{value:this.stockProduct.buy_price, disabled:true}],
				discount : [{value:this.stockProduct.discount, disabled:true}],
				description : [this.stockProduct.description],
	...
	onSubmit(withBack: boolean = false) {
		...
		if (editedStockProduct._id) {
			this.updateStockProduct(editedStockProduct, withBack);
			return;
		}
	...
	addStockProduct( _stockProduct: StockProductModel, withBack: boolean = false) {
		const addSubscription = this.service.createStockProduct(_stockProduct).subscribe(
			...
			err => {
				...
				{
					//console.error(err);
					const message = 'Error while updating Product | ' + err.statusText;
	...
	updateStockProduct(_stockProduct: StockProductModel, withBack: boolean = false) {
		const addSubscription = this.service.updateStockProduct(_stockProduct).subscribe(
			res => {
	...
	getComponentTitle() {
		...
		if (!this.stockProduct || !this.stockProduct._id) {
			return result;
		}
