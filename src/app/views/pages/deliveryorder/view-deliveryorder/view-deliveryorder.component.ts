...
export class ViewDoComponent implements OnInit, OnDestroy {
	...
	createForm() {
		...
		this.deliveryorderForm = this.deliveryorderFB.group({
			...
			attachment: [this.deliveryorder.attachment],
	...
	onSubmit(withBack: boolean = false) {
		...
		if (costName === "" || cost === "") {
			if (this.deliveryorder.status === 'surv-work-done') {
				const message = `Complete contents of CostName and Cost`;
				this.layoutUtilsService.showActionNotification(message, MessageType.Create, 5000, true, true)
				return
			}
		...
		if (this.deliveryorder.status === "wait-sched-surv") {
		...
		if (this.deliveryorder.status === "wait-sched-wo") {
	...
	prepareDeliveryorder(): DeliveryorderModel {
		const controls = this.deliveryorderForm.controls;
	...
	updateDeliveryorder(_deliveryorder: DeliveryorderModel, withBack: boolean = false) {
		const addSubscription = this.service.updateDeliveryorderUpd(_deliveryorder, this.deliveryorder._id).subscribe(
			res => {
			...
			err => {
				console.error(err);
				const message = 'Error while adding deliveryorder | ' + err.statusText;
